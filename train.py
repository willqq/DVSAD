...
class Train_fivefold:
    ...
    def __init__(self, args, fold_num=0):
        ...
        current_path = os.path.join(self.args.save_model_path, 'fold%d'%fold_num)
    ...
    def main(self):
        ...
        self.normalizer = Normalizer(dfs=[data_pre[i][0] for i in range(len(data_pre))],
        ...
        try:
            ...
            while self.current_epoch <= self.args.epochs:
                ...
                for batch in data_loader:
                    ...
                    for i in range(1000):
                        batch_ += 0.01 * torch.randn_like(batch_).cuda()
...
def save_features_info(feature_path, batch, iteration, log_p, mean, target, raw_ts):
    """
    save features
    """
    ...
    with open(dict_path, "wb") as f:
        ...
        batch[1].update({'original_time_series': target})
        batch[1].update({'rec_time_series': log_p})
        batch[1].update({'raw_time_series': raw_ts})
...
def extract(data_loader, model, data_task, feature_path, p_bar, noise_scale, variable_length):
    ...
    for batch in data_loader:
        ...
        log_p, mean, log_v, z, mean_pred = model(batch_, 
                                                 encoder_filter=data_task.encoder_filter,
        ...
        save_features_info(feature_path, batch, iteration, log_p, mean, target, batch_)
        p_bar.update(1)
        iteration += 1
